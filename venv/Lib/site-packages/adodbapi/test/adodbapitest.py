...
class CommonDBTests(unittest.TestCase):
    ...
    def helpTestDataType(self,sqlDataTypeString,
        ...
        elif DBAPIDataTypeString == 'STRING':
        ...
        for fldId, inParam in enumerate(inputs, start=2):
            try:
                crsr.execute(
    ...
    def testDataTypeDate(self):
        dt = "timestamp" if self.getEngine() == 'PostgreSQL' else "datetime"
    ...
    def testFetchAll(self):
        ...
        rs=crsr.fetchall()
        assert len(rs)==9
        for i, row in enumerate(rs[3:-2], start=3): #should have rowid 3..6
            assert row[0]==i
        self.helpRollbackTblTemp()
    ...
    def testExecuteMany(self):
        ...
        if crsr.rowcount==-1:
            print(
                f"{self.getEngine()} Provider does not support rowcount (on .executemany())"
            )
    ...
    def testRowCountNoRecordset(self):  
        ...
        if crsr.rowcount==-1:
            print(f"{self.getEngine()} Provider does not support rowcount (on DELETE)")
        else:
    ...
    def testFormatParamstyle(self):
        ...
        for fldId, inParam in enumerate(inputs, start=3):
            sql = "INSERT INTO xx_" + \
                      config.tmp + \
            ...
            try:
                ...
                raise
            crsr.execute(
                f"SELECT fldData, fldConst FROM xx_{config.tmp} WHERE %s=fldID",
                [fldId],
            )
    ...
    def testNamedParamstyle(self):
        ...
        for fldId, inParam in enumerate(inputs, start=11):
            try:
                crsr.execute(
                ...
                raise
            crsr.execute(
                f"SELECT fldData FROM xx_{config.tmp} WHERE fldID=:Id",
                {'Id': fldId},
            )
    ...
    def testAutomaticParamstyle(self):
        ...
        for fldId, inParam in enumerate(inputs, start=3):
            try:
                ...
                raise
            crsr.execute(
                f"SELECT fldData, fldConst FROM xx_{config.tmp} WHERE ?=fldID",
                [fldId],
            )
        ...
        for fldId, inParam in enumerate(inputs, start=11):
            try:
                ...
                raise
            crsr.execute(
                f"SELECT fldData FROM xx_{config.tmp} WHERE :Id=fldID",
                {'Id': fldId},
            )
    ...
    def testAutoRollback(self):
        ...
        assert (
            row is None
        ), f'cursor.fetchone should return None if a query retrieves no rows. Got {repr(row)}'
