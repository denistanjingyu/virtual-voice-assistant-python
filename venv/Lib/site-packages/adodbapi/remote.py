...
__version__ = '2.6.0.4'
version = f'adodbapi.remote v{__version__}'
...
def connect(*args, **kwargs):    # --> a remote db-api connection object
    ...
    if 'proxy_port' not in kwargs:
        try:
            pport = os.environ['PROXY_PORT']
        except KeyError:
            pport = 9099
        kwargs['proxy_port'] = pport
    if 'proxy_host' not in kwargs or not kwargs['proxy_host']:
        try:
    ...
    while auto_retry:
        try:
            ...
            if auto_retry:
                ...
                raise api.DatabaseError(f'Cannot create connection to={ado_uri}')
        ...
        try:
            ...
            if auto_retry:
                ...
                raise api.DatabaseError(
                    f'Cannot create ADO connection object using={conn_uri}'
                )
    ...
    while auto_retry:
        try:
            ...
            if auto_retry:
                ...
                raise api.DatabaseError(
                    f'Pyro error creating connection to/thru={repr(kwargs)}'
                )
...
def fix_uri(uri, kwargs):
    ...
    return kwargs['proxy_host'].join(s) if len(s) > 1 else uri
...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker):
        ...
        if ret is not True:
            self._raiseConnectionError(
                api.OperationalError, f'Proxy returns error message={repr(ret)}'
            )
    ...
    def commit(self):
        ...
        if not self.supportsTransactions:
            return
        if result := self.proxy.commit():
            self._raiseConnectionError(
                api.OperationalError, f'Error during commit: {result}'
            )
    ...
    def _rollback(self):
        ...
        """
        if result := self.proxy.rollback():
            self._raiseConnectionError(
                api.OperationalError, f'Error during rollback: {result}'
            )

    def __setattr__(self, name, value):
        if name in ('paramstyle', 'timeout', 'autocommit') and self.proxy:
    ...
    def getIndexedValue(self, index):
        return self.proxy.get_attribute_for_remote(index)
...
class Cursor(object):
    ...
    def execute(self, operation, parameters=None):
        ...
        if result := self.proxy.crsr_execute(self.id, operation, fp):
    ...
    def fetchmany(self, size=None):
        try:
            ...
            return api.SQLrows(self.rs, len(self.rs), self) if self.rs else []
