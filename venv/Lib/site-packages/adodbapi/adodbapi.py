...
__version__ = '2.6.2.0'
version = f'adodbapi v{__version__}'
...
def format_parameters(ADOparameters, show_value=False):
    ...
    try:
        if show_value:
            desc = [
                (
                    ...
                    % (
                        ...
                        adc.adTypeNames.get(
                            p.Type, f'{str(p.Type)} (unknown type)'
                        ),
                        p.Size,
                        p.Value,
                        p.Precision,
            ...
            desc = [
                f"Name: {p.Name}, Dir.: {adc.directions[p.Direction]}, Type: {adc.adTypeNames.get(p.Type, f'{str(p.Type)} (unknown type)')}, Size: {p.Size}, Precision: {p.Precision}, NumericScale: {p.NumericScale}"
                for p in ADOparameters
            ]
...
def _configure_parameter(p, value, adotype, settings_known):
    ...
    if adotype in api.adoBinaryTypes:
        ...
        if exponent == 0:
            ...
            p.Precision = max(p.Precision, p.NumericScale)
        else:  # exponent > 0:
            p.NumericScale = 0
...
class Connection(object):
    ...
    def printADOerrors(self):
        if j := self.connector.Errors.Count:
            print('ADO Errors:(%i)' % j)
        for e in self.connector.Errors:
            print(f'Description: {e.Description}')
            print(f'Error: {e.Number} {adc.adoErrors.get(e.Number, "unknown")} ')
...
##   allows the programmer to override the connection's default error handler
    ...
    def _makeDescriptionFromRS(self):
        ...
        for i in range(self.numberOfColumns):
            f = getIndexedValue(self.rs.Fields, i)
            display_size = None if self.rs.EOF or self.rs.BOF else f.ActualSize
    ...
    def format_description(self,d):
        ...
        return f"Name= {d[0]}, Type= {adc.adTypeNames.get(d[1], f'{str(d[1])} (unknown type)')}, DispSize= {d[2]}, IntSize= {d[3]}, Precision= {d[4]}, Scale= {d[5]} NullOK={d[6]}"
                ...
                if self._parameter_names:  # named parameters
                    for i, pm_name in enumerate(self._parameter_names):
                        ...
                        try:
                            ...
                            self._raiseCursorError(api.DataError, f'{_message}->{repr(e.args)}')
